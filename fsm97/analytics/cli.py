...
def run(
    ...
    def write_page(page_path: str, rows: list, nav: dict, period_label: str, always: bool = False):
        ...
        html = templates["analytics"](
            report=report, nav=nav, period_label=period_label, json=json, datetime=datetime
        )
...
def main():
    parser = argparse.ArgumentParser(description="Generate HTML analytics from log CSVs")
    parser.add_argument("--csv-dir", type=Path, default=Path("logs/csv"), help="Input CSV directory (default: logs/csv)")
    parser.add_argument("--templates-dir", type=Path, default=Path("templates"), help="Templates directory (default: templates)")
    parser.add_argument("--manifest", type=Path, default=None, help="Path to manifest.json from site build")
    parser.add_argument("--output-dir", type=Path, default=Path("www-analytics"), help="Output directory (default: www-analytics)")
    parser.add_argument("--base-url", default="", help="Site base URL for self-referrer filtering")
    args = parser.parse_args()

    run(args.csv_dir, args.templates_dir, args.manifest, args.output_dir, args.base_url)
