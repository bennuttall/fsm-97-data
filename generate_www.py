...
BASE    = "/home/ben/.wine/drive_c/FIFA Soccer Manager"
CSV_DIR = f"{BASE}/claude/csv"
OUT_DIR = f"{BASE}/claude/www"

SKILL_COLS = [
    'move_speed','move_agil','move_accl','move_stam','move_str','move_fit',
    'skill_shoot','skill_pass','skill_head','skill_ctrl','skill_drib',
    'cool','awar','tack_deter','tack_skill','flair',
    'gk_kick','gk_thrw','gk_hand','thrw','lead','cons','deter'
]
SKILL_LABELS = {
    'move_speed':'Speed','move_agil':'Agility','move_accl':'Accel',
    'move_stam':'Stamina','move_str':'Strength','move_fit':'Fitness',
    'skill_shoot':'Shooting','skill_pass':'Passing','skill_head':'Heading',
    'skill_ctrl':'Control','skill_drib':'Dribbling',
    'cool':'Cool','awar':'Awareness','tack_deter':'Tack Det','tack_skill':'Tackling',
    'flair':'Flair','gk_kick':'GK Kick','gk_thrw':'GK Throw','gk_hand':'Handling',
    'thrw':'Throw-in','lead':'Leadership','cons':'Consistency','deter':'Determination'
}
SKILL_GROUPS = {
...
# Skills that matter for each position
POS_SKILLS = {
    'GK':  ['gk_hand','gk_kick','gk_thrw','cool','awar','cons'],
    'RB':  ['tack_deter','tack_skill','move_speed','move_stam','move_str','skill_pass','awar','cons'],
    'LB':  ['tack_deter','tack_skill','move_speed','move_stam','move_str','skill_pass','awar','cons'],
    'CD':  ['tack_deter','tack_skill','move_str','skill_head','cool','awar','cons','lead'],
    'RWB': ['tack_deter','tack_skill','move_speed','move_stam','skill_pass','skill_ctrl','skill_drib','awar'],
    'LWB': ['tack_deter','tack_skill','move_speed','move_stam','skill_pass','skill_ctrl','skill_drib','awar'],
    'SW':  ['tack_deter','tack_skill','move_str','skill_head','skill_pass','cool','awar','lead'],
    'DM':  ['tack_deter','tack_skill','move_stam','move_str','skill_pass','awar','cons'],
    'RM':  ['move_speed','move_stam','move_accl','skill_pass','skill_ctrl','skill_drib','flair'],
    'LM':  ['move_speed','move_stam','move_accl','skill_pass','skill_ctrl','skill_drib','flair'],
    'AM':  ['skill_pass','skill_ctrl','skill_drib','skill_shoot','awar','flair','cool'],
    'RW':  ['move_speed','move_accl','skill_drib','skill_pass','skill_shoot','flair'],
    'LW':  ['move_speed','move_accl','skill_drib','skill_pass','skill_shoot','flair'],
    'FR':  ['skill_pass','skill_ctrl','skill_drib','skill_shoot','flair','awar','cool'],
    'FWD': ['skill_shoot','skill_head','skill_ctrl','skill_drib','move_speed','move_accl','cool'],
    'SS':  ['skill_shoot','skill_pass','skill_ctrl','skill_drib','move_speed','cool','flair'],
}

def pos_rating(skill_row, position):
    cols = POS_SKILLS.get(position, SKILL_COLS)
    vals = [int(skill_row.get(c, 0) or 0) for c in cols if c in skill_row]
...
teams_raw    = load_csv('teams.csv')
players_raw  = load_csv('players.csv')
skills_raw   = load_csv('player_skills.csv')
positions_raw= load_csv('positions.csv')
countries_raw= load_csv('countries.csv')

# Index structures
teams_by_slug   = {slug(t['team']): t for t in teams_raw}
league_names    = list(dict.fromkeys(t['league'] for t in teams_raw))
...
skills_by_player = {}
for p, s in zip(players_raw, skills_raw):
    key = (p['first_name'], p['last_name'], p['team'])
    players_by_team[p['team']].append(p)
    skills_by_player[key] = s

players_by_nationality = defaultdict(list)
for p in players_raw:
    players_by_nationality[p['nationality']].append(p)

players_by_position = defaultdict(list)
for p in players_raw:
    players_by_position[p['position']].append(p)

# Precompute position-specific rating for every player
prating = {}  # (first, last, team) -> float
for p, s in zip(players_raw, skills_raw):
    key = (p['first_name'], p['last_name'], p['team'])
    prating[key] = pos_rating(s, p['position'])

def pr(p):
    """Return position rating for a player dict."""
    return prating.get((p['first_name'], p['last_name'], p['team']), 0.0)

positions_info = {r['abbreviation']: r for r in positions_raw}
pos_order = ['GK','RB','LB','CD','RWB','LWB','SW','DM','RM','LM','AM','RW','LW','FR','FWD','SS']

# Stadium index — detect shared stadiums
stadium_to_teams = defaultdict(list)
for t in teams_raw:
    sname = t['stadium'].strip()
    if sname and not re.match(r'^XX\d+$', sname):
        stadium_to_teams[sname].append(t)

max_cap = max(int(t['capacity']) for t in teams_raw if t['capacity'])
...
def page(title, body, depth=1, active=None, breadcrumb='', header_title=None, header_sub=''):
    ...
    nav_links = [
        ...
        ('Leagues',       f'{prefix}leagues/index.html'),
        ('Teams',         f'{prefix}teams/index.html'),
        ('Stadiums',      f'{prefix}stadiums/index.html'),
        ('Positions',     f'{prefix}positions/index.html'),
        ('Nationalities', f'{prefix}nationalities/index.html'),
        ('Stats',         f'{prefix}stats/index.html'),
        ('Trivia',        f'{prefix}trivia/index.html'),
    ]
    nav_html = '\n'.join(
...
{hdr}
<main>{body}</main>
<footer>FIFA Soccer Manager 97 · Data extracted from SM97.DAT</footer>
<script>
...
def bc(parts, prefix=''):
    ...
    bits = ['<a href="{}{}">Home</a>'.format(prefix, 'index.html')]
...
PLAYER_TRIVIA = {
    ...
    ('Roberto','Carlos'): "Real Madrid's marauding left back, famous for the seemingly impossible free-kick against France in 1997 — struck with such swerve that the ball appeared to curve around the wall before bending back.",
    ('Ronaldo',''): "The Brazilian phenomenon joined Barcelona from PSV in 1996 and won FIFA World Player of the Year in his debut season. Moved to Inter Milan at the end of 1996–97.",
    ...
    ('Gianluigi','Pagliuca'): "Inter Milan's veteran goalkeeper who famously became the first goalkeeper sent off in World Cup history, against Norway at USA 94.",
}
...
STADIUM_TRIVIA = {
    ...
    'the-cellnet-riverside-stadiu': "Middlesbrough's gleaming new stadium opened in August 1995, replacing the ageing Ayresome Park. One of the first post-Taylor Report football-specific stadiums in England.",
    ...
    'westfalsenstadion': "Borussia Dortmund's famous yellow wall — the Südtribüne — holds 25,000 standing fans, making it the largest single terrace in European football.",
}
...
def team_url(team_name, prefix=''):
    return f"{prefix}teams/{slug(team_name)}.html"

def league_url(league_name, prefix=''):
    return f"{prefix}leagues/{slug(league_name)}.html"

def stadium_url(stadium_name, prefix=''):
    if re.match(r'^XX\d+$', stadium_name.strip()):
        return None
    return f"{prefix}stadiums/{slug(stadium_name)}.html"

def nat_url(nationality, prefix=''):
    if not nationality:
        return None
    return f"{prefix}nationalities/{slug(nationality)}.html"

def pos_url(position, prefix=''):
    if not position:
        return None
    return f"{prefix}positions/{slug(position)}.html"
...
def squad_table(team_name, prefix, show_league=False):
    ...
    for p in players:
        ...
        nat_link = tlink(p['nationality'], nat_url(p['nationality'], prefix))
        pos_link = f'<a href="{pos_url(p["position"], prefix)}" class="pos">{h(p["position"])}</a>' if p['position'] else ''
        avg = pr(p)
        avg_cell = f'<span class="{rating_class(avg)}">{avg:.1f}</span>'
        skill_cells = ''
        if s:
...
def make_home():
    ...
    highlights = f'''<div class="highlights">
      ...
      <div class="hl">
        <div class="hl-val">{pr(best_player):.1f}</div>
      ...
      <div class="hl">
        ...
        <div class="hl-sub"><a href="stadiums/{slug(biggest["stadium"])}.html">{h(biggest["stadium"])}</a></div>
      </div>
    </div>'''
    ...
    for lg in league_names:
        ...
        league_cards += f'''<a href="leagues/{slug(lg)}.html" style="text-decoration:none">
          <div class="card">
            <h3>{h(lg)}</h3>
...
def make_leagues():
    ...
    for lg in league_names:
        ...
        rows += f'<tr><td><a href="{slug(lg)}.html">{h(lg)}</a></td><td class="num">{len(teams)}</td><td class="num">{pc}</td></tr>'
    body = f'''<table><thead><tr><th>League</th><th class="num">Teams</th><th class="num">Players</th></tr></thead>
    ...
    for lg in league_names:
        ...
        for t in teams:
            ...
            stad  = tlink(t['stadium'], f'../stadiums/{slug(t["stadium"])}.html') if slink else h(t['stadium'])
        ...
        for p in top_players:
            player_rows += f'''<tr>
              ...
              <td class="num"><span class="{rating_class(pr(p))}">{pr(p):.1f}</span></td>
            </tr>'''
        ...
        <table id="teams-{slug(lg)}">
          <thead><tr><th data-sort>Club</th><th data-sort>Nickname</th><th data-sort>Stadium</th><th class="num" data-sort>Capacity</th><th data-sort>Manager</th><th class="num" data-sort>Squad Rating</th></tr></thead>
        ...
        write(f"{OUT_DIR}/leagues/{slug(lg)}.html",
              page(lg, body, depth=1, active='Leagues',
                   header_title=lg,
                   header_sub=f"{len(teams)} clubs",
                   breadcrumb=bc([('Leagues','index.html'),(lg,None)])))
...
def make_teams():
    ...
    <table id="all-teams">
      <thead><tr><th>Team</th><th>League</th><th>City</th><th class="num">Capacity</th><th class="num">Squad Rating</th></tr></thead>
      <tbody>{rows}</tbody>
    </table>'''
    ...
    for t in teams_raw:
        ...
        stad_html = tlink(t['stadium'], f'../stadiums/{slug(t["stadium"])}.html') if stad_link else h(t['stadium'])

        trivia_html = ''
        ct = CLUB_TRIVIA.get(tslug) or CLUB_TRIVIA.get(slug(t['team']))
        if ct:
            trivia_html = f'<div class="trivia">{h(ct)}</div>'

        meta = f'''<div class="cards" style="margin-bottom:1rem">
          <div class="card"><div class="stat">{h(t["league"])}</div><div class="label"><a href="../leagues/{slug(t["league"])}.html">View league</a></div></div>
          <div class="card"><div class="stat">{stad_html}</div><div class="label">Stadium · {int(t["capacity"]):,} cap</div></div>
          <div class="card"><div class="stat">{h(t["city"] or "—")}</div><div class="label">City</div></div>
...
def make_stadiums():
    ...
    for sname, ts in all_sorted:
        ...
        cities = ', '.join(set(t['city'] for t in ts if t['city']))
    ...
    <table id="stad-idx">
      <thead><tr><th>Stadium</th><th>City</th><th>Club(s)</th><th class="num">Capacity</th></tr></thead>
      <tbody>{rows}</tbody>
    </table>'''
    ...
    for sname, ts in named.items():
        ...
        if tv:
            trivia_html = f'<div class="trivia">{h(tv)}</div>'

        club_cards = ''
        for t in ts:
            pc  = len(players_by_team[t['team']])
            avg = (sum(pr(p) for p in players_by_team[t['team']]) / pc) if pc else 0
            club_cards += f'''<a href="../teams/{slug(t["team"])}.html" style="text-decoration:none">
              <div class="card">
                <h3>{h(t["team"])}</h3>
                <div class="label"><a href="../leagues/{slug(t["league"])}.html">{h(t["league"])}</a></div>
        ...
        meta = f'''<div class="highlights" style="margin-bottom:1rem">
          ...
          <div class="hl"><div class="hl-val">{h(cities or "—")}</div><div class="hl-lbl">City</div></div>
        ...
        write(f"{OUT_DIR}/stadiums/{sslug}.html",
              page(sname, body, depth=1, active='Stadiums',
                   header_title=sname,
                   header_sub=f"Capacity {cap:,} · {cities}",
                   breadcrumb=bc([('Stadiums','index.html'),(sname,None)])))
...
def make_nationalities():
    ...
    for nat, pp in nats_sorted:
        ...
        avg = sum(pr(p) for p in pp) / len(pp)
        rows += f'''<tr>
          <td><a href="{slug(nat)}.html">{h(nat)}</a></td>
          <td class="num">{len(pp)}</td>
          <td class="num"><span class="{rating_class(avg)}">{avg:.1f}</span></td>
        </tr>'''
    ...
    for nat, pp in nats_sorted:
        ...
        write(f"{OUT_DIR}/nationalities/{slug(nat)}.html",
              page(nat, body, depth=1, active='Nationalities',
                   header_title=f"{nat} Players",
                   header_sub=f"{len(pp)} players",
                   breadcrumb=bc([('Nationalities','index.html'),(nat,None)])))
...
def make_stats_index():
    body = '''<div class="cards">
      ...
      <a href="best-of.html" style="text-decoration:none"><div class="card"><h3>🏆 Best Of All</h3><div class="label">Records, superlatives and extremes</div></div></a>
    </div>'''
...
def make_stats_top_players():
    ...
    {by_pos_html}'''
    write(f"{OUT_DIR}/stats/top-players.html",
          page("Top Players", body, depth=1, active='Stats',
               header_title="Top Players",
               breadcrumb=bc([('Stats','index.html'),('Top Players',None)])))
...
def make_stats_skill_leaders():
    ...
    write(f"{OUT_DIR}/stats/skill-leaders.html",
          page("Skill Leaders", sections, depth=1, active='Stats',
               ...
               breadcrumb=bc([('Stats','index.html'),('Skill Leaders',None)])))
...
def make_stats_stadiums():
    ...
    for i, (sname, ts) in enumerate(named, 1):
        ...
        city  = ', '.join(set(t['city'] for t in ts if t['city']))
        rows += f'''<tr>
          <td class="num">{i}</td>
    ...
    body = f'''<table><thead><tr><th class="num">#</th><th>Stadium</th><th>City</th><th>Club(s)</th><th class="num">Capacity</th></tr></thead>
    <tbody>{rows}</tbody></table>'''
    write(f"{OUT_DIR}/stats/stadiums.html",
          page("Stadium Rankings", body, depth=1, active='Stats',
               header_title="Stadiums by Capacity",
               breadcrumb=bc([('Stats','index.html'),('Stadium Rankings',None)])))
...
def make_stats_squads():
    ...
    write(f"{OUT_DIR}/stats/squads.html",
          page("Squad Rankings", body, depth=1, active='Stats',
               header_title="Squad Rankings",
               breadcrumb=bc([('Stats','index.html'),('Squad Rankings',None)])))
...
def make_stats_nationalities():
    ...
    write(f"{OUT_DIR}/stats/nationalities.html",
          page("Nationality Stats", body, depth=1, active='Stats',
               header_title="Nationality Statistics",
               breadcrumb=bc([('Stats','index.html'),('Nationalities',None)])))
...
def make_stats_best_of():
    ...
    for pos in pos_order:
        ...
        pos_rows += f'''<tr>
          ...
          <td class="num"><span class="{rating_class(pr(best))}">{pr(best):.1f}</span></td>
        </tr>'''
...
def make_trivia():
    ...
    <div class="cards">
      <a href="players.html" style="text-decoration:none"><div class="card"><h3>🧑 Players</h3><div class="label">Notable player stories</div></div></a>
      <a href="stadiums.html" style="text-decoration:none"><div class="card"><h3>🏟️ Stadiums</h3><div class="label">The grounds and their histories</div></div></a>
      <a href="clubs.html" style="text-decoration:none"><div class="card"><h3>⚽ Clubs</h3><div class="label">Club stories from the era</div></div></a>
    </div>'''
    ...
    for (first, last), text in PLAYER_TRIVIA.items():
        ...
        if matches:
            p = matches[0]
            link = f'<a href="../{player_anchor(first, last, p["team"])}">{h(first)} {h(last)}</a>'
            team_link = f'<a href="../teams/{slug(p["team"])}.html">{h(p["team"])}</a>'
            nat_link  = tlink(p["nationality"], f'../nationalities/{slug(p["nationality"])}.html')
            avg = pr(p)
            badge = f'<span class="{rating_class(avg)}">{avg:.1f}</span>'
            header = f'{link} · {team_link} · {nat_link} · <a href="../positions/{slug(p["position"])}.html" class="pos">{h(p["position"])}</a> · {badge}'
        else:
            header = f'{h(first)} {h(last)}'
        sections += f'<div style="margin-bottom:1.5rem"><h3>{header}</h3><div class="trivia">{h(text)}</div></div>'

    write(f"{OUT_DIR}/trivia/players.html",
          page("Player Trivia", sections, depth=1, active='Trivia',
               header_title="Player Trivia",
               breadcrumb=bc([('Trivia','index.html'),('Players',None)])))
    ...
    for sslug, text in STADIUM_TRIVIA.items():
        ...
        if match_name:
            ...
            cap = int(ts[0]['capacity'] or 0)
            club_links = ', '.join(f'<a href="../teams/{slug(t["team"])}.html">{h(t["team"])}</a>' for t in ts)
            stad_link = f'<a href="../stadiums/{sslug}.html">{h(match_name)}</a>'
            header = f'{stad_link} · {club_links} · {cap:,} cap'
        else:
            header = h(sslug.replace('-',' ').title())
        sections += f'<div style="margin-bottom:1.5rem"><h3>{header}</h3><div class="trivia">{h(text)}</div></div>'

    write(f"{OUT_DIR}/trivia/stadiums.html",
          page("Stadium Trivia", sections, depth=1, active='Trivia',
    ...
    write(f"{OUT_DIR}/trivia/clubs.html",
          page("Club Trivia", sections, depth=1, active='Trivia',
               header_title="Club Trivia",
               breadcrumb=bc([('Trivia','index.html'),('Clubs',None)])))
